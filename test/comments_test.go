...
func TestComments_Add_ByAuthenticatedUser(t *testing.T) {
	...
	runTest(t, func() {
		...
		var out commentResp
		step("POST a comment as an authenticated user", func() {
...
func TestComments_Add_Unauthenticated(t *testing.T) {
	...
	runTest(t, func() {
		...
		attach("response", resp)
		// POST is not GET, so the JWT middleware runs and rejects the missing token.
...
func TestComments_Get_Public(t *testing.T) {
	...
	runTest(t, func() {
		...
		step("GET the comment list with no auth (public read)", func() {
...
func TestComments_Delete_ByNonAuthor(t *testing.T) {
	...
	runTest(t, func() {
		...
		var resp apiResp
		step("DELETE Alice's comment while authenticated as Bob", func() {
			resp = ta.doReq(t, http.MethodDelete,
				pathComment(created.Article.Slug, itoa(cm.Comment.ID)), nil, bob)
			attach("response", resp)
		})
		// DeleteComment checks cm.UserID != caller → 401 "unathorized action".
...
func TestComments_Delete_NonNumericID(t *testing.T) {
	...
	runTest(t, func() {
		...
		attach("response", resp)
		// strconv.ParseUint fails before any store lookup → 400.
		require.Equal(t, http.StatusBadRequest, resp.status)
	}, allure.Feature(featComments), allure.Story(storyDelete),
